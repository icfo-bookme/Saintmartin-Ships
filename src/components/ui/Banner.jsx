...
const Banner = ({ imageUrl, ship, title, route, heightClass }) => {
          ...
          <div className="flex flex-row flex-wrap gap-2 sm:gap-3 mt-4">
            ...
            {['01841666644', '01841333322'].map((number, index) => (
              <a
                key={index}
                href={`tel:${number}`}
                className="backdrop-blur-sm text-white border border-white font-medium 
               px-2 py-1 lg:px-4 lg:py-2 rounded-3xl shadow-lg 
               hover:bg-white/20 transition-all duration-300 
               text-[11px] md:text-sm inline-block"
              >
                {number}
              </a>
            ))}
            ...
            onClick={() => {
              if (typeof window !== "undefined") {
                ...
                window.dataLayer.push({
                  ...
                  click_location: "banner_whatsapp_button",
                  phone: "+8801841666644",
                });
              }
              window.open('https://wa.me/+8801841666644', '_blank');
            }}
